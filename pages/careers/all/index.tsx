...
import CareerFilterButtonList from '@components/careers/CareerFilterButtonList';
...
const CareersAll = observer(() => {
    ...
    throttle(
      (
        ...
        careerStore
          ...
          .then(() => {
            const { data, totalCount } = toJS(careerStore.recruitmentListData);
  ...
  useEffect(() => {
    ...
    fetchData(searchText, filterValue, sort, null, authPayload?.access_token);
  }, [shouldFetch]);
  ...
  const handleRefetch = (filters = filterValue, newSort = sort) => {
    setCareerSliceData([]);
    setHasMore(true);
    setHasListRendered(false);
    fetchData(searchText, filters, newSort, null, authPayload?.access_token);
  };

  const handleSortChange = (sortValue: 'UPLOAD' | 'DEADLINE') => {
    setSort(sortValue);
    handleRefetch(filterValue, sortValue);
  };
  ...
  const handleFilterDataUpdate = (newFilter: {}) => {
    setFilterValue(newFilter);
    handleRefetch(newFilter);
  };
  ...
  const onIntersect = useCallback(() => {
    ...
    fetchData(
      ...
      filterValue,
      sort,
      toJS(careerStore.recruitmentListData.cursor),
        ...
        <CareerPageLayout>
          <CareerAllSectionContainer>
            <SectionHeader>
              <SectionTitleContainer>
                <SectionTitleAndSearch>
                  {!(searchSelected && screenSize === 'small') && (
                    <SectionTitle>전체 채용 공고</SectionTitle>
                  )}
                  <CareerSearchButton
                    value={searchText}
              ...
              <CareerFilterButtonList onUpdate={handleFilterDataUpdate} />
            </SectionHeader>
