...
export default async function getAllByTimeTable(
    timeTableUuid: string,
    accessToken: string
): Promise<ApiResponse<TimeTableCourseGradeViewDto[] | null>> {
    const REST_API_ENDPOINT = process.env.NEXT_PUBLIC_REST_API_ENDPOINT;
    const requestUrl = `${REST_API_ENDPOINT}/api/v1/timetable-course-grade/get-by-timetable?timeTableUuid=${timeTableUuid}`;
    ...
    try {
        ...
        let message = '시간표 과목 성적 조회에 실패했습니다.';
