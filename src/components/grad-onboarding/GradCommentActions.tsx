...
import UserBlock from '../community/popup/UserBlock';
import CommentDelete from '../community/popup/CommentDelete';
import UserReport from '../community/popup/UserReport';
import { deleteLike, registerBlock, registerLike } from '@api/community/userInteraction';
...
export default function GradCommentActions({
    commentId,
    authorId,
    ...
    const handleLike = async () => {
        ...
        try {
            if (isLiked) {
                // 좋아요 취소
                const result = await deleteLike('ARTICLE_REVIEW', commentId, undefined, accessToken);
                if (result.success) {
                    onRefresh?.();
                } else {
                ...
                // 좋아요 등록
                const result = await registerLike('ARTICLE_REVIEW', commentId, accessToken);
                if (result.success) {
                    onRefresh?.();
                } else {
                    ...
                    <div className="absolute top-8 right-0 bg-white rounded-xl shadow-lg border border-gray-20 w-32 py-2 z-20">
                        {isOwn ? (
                            ...
                            <div className='flex flex-col'>
                                <Link
                                    href={`/my/alarm/message/send?uuid=${commentId}&partnerUuid=${authorId}&type=ARTICLE_REVIEW`}
                                    onClick={(e) => {
                                        // 데스크탑이면 기본 이동 막기
                                        if (!isMobile) {
                                            e.preventDefault();
                                            HeaderStore.setIsSendMessageOpen(true);
                                            HeaderStore.setType("ARTICLE_REVIEW")
