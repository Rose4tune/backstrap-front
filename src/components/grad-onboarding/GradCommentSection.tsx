...
import { Author, Comment } from '../community/post/Post.types';
import getCommentsByPaging from '@api/community/comments/getCommentsByPaging';
import CommentForm from '../community/comments/CommentForm';
import GradCommentItem from './GradCommentItem';
...
export default function GradCommentSection({ uuid, postAuthorId }: CommentSectionProps) {
    ...
    return (
        <div className="flex pt-5 flex-col w-full">
            ...
            {/* 댓글 목록 */}
            <div className={`flex flex-col gap-1 relative rounded-xl ${accessToken ? 'min-h-[80px]' : 'min-h-[200px]'}`}>
                {!accessToken &&
                    (
                ...
                {/* 실제 댓글 */}
                <div
                    className={`flex flex-col gap-1 ${!accessToken && "blur-md"
                        } ${!isMobile && "px-5"}`}
                >
                    {comments.length === 0 && accessToken ? (
                        <div className="flex flex-1 items-center justify-center min-h-[60px]">
                            <span className="text-med-14 text-gray-70">
                                아직 댓글이 없습니다.
                            </span>
                        </div>
                    ) : (
