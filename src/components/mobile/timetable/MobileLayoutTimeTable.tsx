...
export default function LayoutTimeTable({
    ...
    setIsMobileSettingModalOpen,
    setIsMobileListModalOpen
}: BottomTabProps) {
    ...
    return (
        <>
            <nav
                className={`
          flex flex-col-reverse fixed top-0 left-0 right-0 z-50 bg-white border-b-[1px] border-gray-30 w-full p-5 max-w-[550px] mx-auto
        `}
            >
                <div className="flex flex-row items-center justify-between">
                    {mobileSimple ? (
                        <DropdownArrowIcon onClick={() => router.back()} className="w-5 h-5 rotate-180 text-gray-50" />
                    ) : currentActiveTab.path === 'home' ? <Logo /> : currentActiveTab.path === "my/timetable" ?
                        <div className='flex gap-2 items-end'>
                            <span className='text-semibold-18 text-gray-90 truncate'>{selectedTimeTableEntity ? selectedTimeTableEntity?.name : '시간표'}</span>
                            <span className='text-semibold-14 text-normal'>{formatSemesterLabel({ semester: selectedTimeTableEntity?.semester, year: selectedTimeTableEntity?.year })}</span>
                        </div> : <div className='text-bold-16'>{currentActiveTab.label}</div>}
                    <div className='flex fex-row gap-x-5'>
                        {currentActiveTab.path === "my/timetable" ?

                            <>
                                <button onClick={() => setIsMobileAddModalOpen?.(true)}>
                                    <PlusIcon className='w-6 h-6 text-gray-50' />
                                </button>
                                <button onClick={() => setIsMobileSettingModalOpen?.(true)}>
                                    <SettingIcon className='w-6 h-6 text-gray-50' />
                                </button>
                                <button onClick={() => setIsMobileListModalOpen?.(true)}>
                                    <DrawerIcon className="w-6 h-6 text-gray-50" />
                                </button>
                            </>
                            :
                            <>
            ...
            <nav
                className={`
          fixed bottom-0 left-0 right-0 z-50 bg-white shadow-[0px_0px_12px_0px_rgba(0,0,0,0.1)] w-full max-w-[550px] mx-auto
          ${className}
        `}
            >
                {postEditPopup &&
