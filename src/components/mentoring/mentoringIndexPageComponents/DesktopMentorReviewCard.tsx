...
//idx는 key 보장을 위해 넘김
export default function DesktopMentorReviewCard({ review }: MentorReviewCardProps) {
  const router = useRouter();
  return (
    <div
      className="inline-block px-[28px] pt-[28px] border-[1px] border-gray-30 rounded-[20px]"
    ...
    >
      ...
      <div className="flex justify-between mb-[8px]">
        <p className="text-bold-16 text-gray-90">{review.mentorName}</p>
      ...
      <div className="flex gap-[8px] items-center mb-[12px]">
        <div className="flex gap-[4px]">
          {[...Array(Math.ceil(Number(review.rating)))].map((__, idx) => (
            <MentorRatingIcon key={idx} width={16} height={16} />
          ))}
        </div>
      ...
      {/* 리뷰 내용 */}
      <p className="whitespace-pre-line text-med-14 text-gray-60 mb-[12px]">
        {review.content}
      </p>
