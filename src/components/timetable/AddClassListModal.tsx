...
function AddClassListModal({
    ...
    return (
        <>
            ...
            />
            <div className="fixed z-[1001] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[835px] pl-[20px] pb-[20px] bg-white rounded-[16px] overflow-hidden scroll-custom h-[500px]">
                <div className="flex w-full justify-between pt-[20px] pb-[14px]">
                    <label className="text-gray-90 text-semibold-16 ">수업 찾기</label>
                    {/* 닫기 버튼 */}
                    <button
                        onClick={() => setIsClassListModalOpen(false)}
                        className="pr-[20px]"
                    >
                        <CancelIcon className="w-[20px] h-[20px] text-gray-70" />
                    </button>
                </div>

                <div className="flex">
                    {/* 왼쪽 필터 영역 */}
                    <div className="w-[335px] p-5 space-y-2">
                        <SearchInput placeholder={"검색어를 입력해주세요."} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                        <DropdownInput
                            title="캠퍼스"
                    ...
                    {/* 오른쪽 수업 목록 영역 */}
                    <div className="flex-1 overflow-y-auto max-h-[435px]">
...
export default observer(AddClassListModal)