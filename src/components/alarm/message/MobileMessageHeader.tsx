...
const MobileMessageHeader: React.FC<Props> = ({ name, school, isAnynomous, isOut, isReportedByMe, partnerUuid, roomUuid}) => {
    ...
    return (
        <>
            <div className="fixed top-0 w-full bg-white flex items-center justify-between px-5 pt-[53px] pb-[20px] border-b border-gray-30">
                <button
                    onClick={() => router.push('/my/alarm/message')}
                    className="w-[26px] h-[26px] flex items-center justify-center">
                    <ArrowLeftIcon width={20} height={20} className="text-gray-50" />
                </button>

                <div className="flex gap-2 items-center">
                    <span className="text-semibold-16 text-gray-90">{isOut ? '(알 수 없음)' : isAnynomous ? '익명의 끈' : name}</span>
                    {!isAnynomous && !isOut &&
            ...
            {showModal && <MobileMessageModal roomUuid={roomUuid} partnerUuid={partnerUuid} isOut={isOut} isReportedByMe={isReportedByMe} onClose={() => setShowModal(false)} />}
