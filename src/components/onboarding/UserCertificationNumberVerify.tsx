import { verifyFindIdCode } from '@apis/onboarding/verifyFindIdCode';
...
interface UserEmailCertificationInterface {
  ...
  phoneNumber: string;
  name: string;
  setFindEmail: Dispatch<React.SetStateAction<string>>;
}

export default function UserCertificationNumberVerification({
  setStep,
  phoneNumber,
  name,
  setFindEmail
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      const response = await verifyFindIdCode(phoneNumber, verifycode);
      if (response.success) {
        if (response.success && response.data) {
          setFindEmail(response.data);
          setStep(3); //STEP_3
        } else {
          alert(response.messages); //추후 처리
        }
      } else {
    ...
    verify();
  }, [values, name, phoneNumber, setStep]);
